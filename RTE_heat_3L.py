...
"""
RTE_heat_3L.py: general solution implementation
"""
...
class rte_heat_3L:
    ...
    def calc_laplace_pre_P3(self,q,phiq):
        ...
        c_p3_rev = np.empty([q.size,phiq.size,3*8],dtype=np.complex)
        ...
        for i in range(q.size):
            ...
            c_p3_rev[i] = calc_c_marshak_3L_P3(
                self.l3,mut3[i][...,None],self.ew3[i,0],self.ev3[i,0],self.eps3[i],
                self.l2,mut2[i][...,None],self.ew2[i,0],self.ev2[i,0],self.eps2[i]*np.exp(-mut3[i]*self.l3)[...,None],
                self.l1,mut1[i][...,None],self.ew1[i,0],self.ev1[i,0],self.eps1[i]*np.exp(-mut3[i]*self.l3-mut2[i]*self.l2)[...,None]
                )

        self.ci_1 = c_p3[...,0:4]
        self.cr_1 = c_p3[...,4:8]
        self.ci_1_rev = c_p3_rev[...,16:20]
        self.cr_1_rev = c_p3_rev[...,20:24]

        self.ci_2 = c_p3[...,8:12]
        self.cr_2 = c_p3[...,12:16]
        self.ci_2_rev = c_p3_rev[...,8:12]
        self.cr_2_rev = c_p3_rev[...,12:16]

        self.ci_3 = c_p3[...,16:20]
        self.cr_3 = c_p3[...,20:24]
        self.ci_3_rev = c_p3_rev[...,0:4]
        self.cr_3_rev = c_p3_rev[...,4:8]

        self.Q1o = self.Q1o_p3
        self.Q2o = self.Q2o_p3
        self.Q3o = self.Q3o_p3

        self.Q1u = self.Q1u_p3
        self.Q2u = self.Q2u_p3
        self.Q3u = self.Q3u_p3
    ...
    def calc_laplace_pre_P1(self,q,phiq):
        ...
        c_p1_rev = np.empty([q.size,phiq.size,3*2],dtype=np.complex)
        ...
        for i in range(q.size):
            ...
            c_p1_rev[i] = calc_c_marshak_3L_P1(
                self.l3,mut3[i][...,None],self.ew3[i,0],self.ev3[i,0],self.eps3[i],
                self.l2,mut2[i][...,None],self.ew2[i,0],self.ev2[i,0],self.eps2[i]*np.exp(-mut3[i]*self.l3)[...,None],
                self.l1,mut1[i][...,None],self.ew1[i,0],self.ev1[i,0],self.eps1[i]*np.exp(-mut3[i]*self.l3-mut2[i]*self.l2)[...,None]
                )
        ...
        self.ci_1_rev = c_p1_rev[...,4,None]
        self.cr_1_rev = c_p1_rev[...,5,None]

        self.ci_2 = c_p1[...,2,None]
        self.cr_2 = c_p1[...,3,None]
        self.ci_2_rev = c_p1_rev[...,2,None]
        self.cr_2_rev = c_p1_rev[...,3,None]

        self.ci_3 = c_p1[...,4,None]
        self.cr_3 = c_p1[...,5,None]
        self.ci_3_rev = c_p1_rev[...,0,None]
        self.cr_3_rev = c_p1_rev[...,1,None]

        self.Q1o = self.Q1o_p1
        self.Q2o = self.Q2o_p1
        self.Q3o = self.Q3o_p1

        self.Q1u = self.Q1u_p1
        self.Q2u = self.Q2u_p1
        self.Q3u = self.Q3u_p1
    ...
    def calc_laplace(self, s, q, phiq, z):
        ...
        detM_rev = calc_M_3L_delta(self.k3, self.k2, self.k1,
                                gamma3, gamma2, gamma1, e3, e2, e1,
                                p3, p2, p1, m3, m2, m1, self.h2, self.h1)
        ...
        d1_rev = np.exp(-mut3*L3-mut2*L2)

        f1_1_rev,f2_1_rev = calc_f(V00_1,self.ci_1_rev,self.cr_1_rev,ew1,eps1,mut1,gamma1,L1,d1_rev)
        f2_1_rev = f2_1_rev*(1.0-self.h1/gamma1)
        ...
        d2_rev = np.exp(-mut3*L3)

        f1_2_rev,f2_2_rev = calc_f(V00_2,self.ci_2_rev,self.cr_2_rev,ew2,eps2,mut2,gamma2,L2,d2_rev)
        ...
        f1_3_rev,f2_3_rev = calc_f(V00_3,self.ci_3_rev,self.cr_3_rev,ew3,eps3,mut3,gamma3,L3)
        f1_3_rev = f1_3_rev*(1.0-self.h2/gamma3)
        ...
        if(z < self.l1):
            ...
            detA_rev = calc_A_3L_delta_q3_z3(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_1_rev, f2_1_rev)
            detB_rev = calc_B_3L_delta_q3_z3(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_1_rev, f2_1_rev)

            detAg = self.Q1o*detA/detM + self.Q1u*detB_rev/detM_rev
            detBg = self.Q1o*detB/detM + self.Q1u*detA_rev/detM_rev

            particular = calc_part(V00_1,self.ci_1,self.cr_1,ew1,eps1,mut1,gamma1,self.k1,L1,z)
            particular_rev = calc_part(V00_1,self.ci_1_rev,self.cr_1_rev,ew1,eps1,mut1,gamma1,self.k1,L1,L1-z,d1_rev)

            partg = self.Q1o*particular + self.Q1u*particular_rev
            ...
            detA_rev = calc_A_3L_delta_q2_z3(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_2_rev, f2_2_rev)
            detB_rev = calc_B_3L_delta_q2_z3(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_2_rev, f2_2_rev)

            detAg = self.Q2o*detA/detM + self.Q2u*detB_rev/detM_rev
            detBg = self.Q2o*detB/detM + self.Q2u*detA_rev/detM_rev
            ...
            detA_rev = calc_A_3L_delta_q1_z3(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_3_rev, f2_3_rev)
            detB_rev = calc_B_3L_delta_q1_z3(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_3_rev, f2_3_rev)

            detAg = self.Q3o*detA/detM + self.Q3u*detB_rev/detM_rev
            detBg = self.Q3o*detB/detM + self.Q3u*detA_rev/detM_rev
            ...
            detA_rev = calc_A_3L_delta_q3_z2(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_1_rev, f2_1_rev)
            detB_rev = calc_B_3L_delta_q3_z2(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_1_rev, f2_1_rev)
            ...
            detA_rev = calc_A_3L_delta_q2_z2(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_2_rev, f2_2_rev)
            detB_rev = calc_B_3L_delta_q2_z2(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_2_rev, f2_2_rev)
            ...
            particular_rev = calc_part(V00_2,self.ci_2_rev,self.cr_2_rev,ew2,eps2,mut2,gamma2,self.k2,L2,Lg-z-L3,d2_rev)

            partg = self.Q2o*particular + self.Q2u*particular_rev
            ...
            detA_rev = calc_A_3L_delta_q1_z2(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_3_rev, f2_3_rev)
            detB_rev = calc_B_3L_delta_q1_z2(
                self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                e3, e2, e1, p3, p2, p1, m3, m2, m1,
                self.h2, self.h1, f1_3_rev, f2_3_rev)
            ...
            detA_rev = calc_A_3L_delta_q3_z1(
                        self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                        e3, e2, e1, p3, p2, p1, m3, m2, m1,
                        self.h2, self.h1, f1_1_rev, f2_1_rev)
            detB_rev = calc_B_3L_delta_q3_z1(
                        self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                        e3, e2, e1, p3, p2, p1, m3, m2, m1,
                        self.h2, self.h1, f1_1_rev, f2_1_rev)
            ...
            detA_rev = calc_A_3L_delta_q2_z1(
                    self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                    e3, e2, e1, p3, p2, p1, m3, m2, m1,
                    self.h2, self.h1, f1_2_rev, f2_2_rev)
            detB_rev = calc_B_3L_delta_q2_z1(
                    self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                    e3, e2, e1, p3, p2, p1, m3, m2, m1,
                    self.h2, self.h1, f1_2_rev, f2_2_rev)
            ...
            detA_rev = calc_A_3L_delta_q1_z1(
                    self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                    e3, e2, e1, p3, p2, p1, m3, m2, m1,
                    self.h2, self.h1, f1_3_rev, f2_3_rev)
            detB_rev = calc_B_3L_delta_q1_z1(
                    self.k3, self.k2, self.k1, gamma3, gamma2, gamma1,
                    e3, e2, e1, p3, p2, p1, m3, m2, m1,
                    self.h2, self.h1, f1_3_rev, f2_3_rev)
            ...
            particular_rev = calc_part(V00_3,self.ci_3_rev,self.cr_3_rev,ew3,eps3,mut3,gamma3,self.k3,L3,Lg-z)

            partg = self.Q3o*particular + self.Q3u*particular_rev
    ...
    def calc_absorbance(self, q, phiq, z):
        ...
        ew1 = np.moveaxis(ew1, 0, -1)
        ev1 = np.moveaxis(ev1, [0,1], [-2,-1])
        eps1 = np.moveaxis(eps1, 0, -1)

        ew2 = np.moveaxis(ew2, 0, -1)
        ev2 = np.moveaxis(ev2, [0,1], [-2,-1])
        eps2 = np.moveaxis(eps2, 0, -1)

        ew3 = np.moveaxis(ew3, 0, -1)
        ev3 = np.moveaxis(ev3, [0,1], [-2,-1])
        eps3 = np.moveaxis(eps3, 0, -1)

        c_p3 = np.empty([q.size,phiq.size,3*8],dtype=np.complex)
        c_p3_rev = np.empty([q.size,phiq.size,3*8],dtype=np.complex)

        for i in range(q.size):
                c_p3[i] = self.calc_c_semi_marshak_3L_P3(
                    L1,mut1[i][...,None],ew1[i,0],ev1[i,0],eps1[i],
                    L2,mut2[i][...,None],ew2[i,0],ev2[i,0],eps2[i]*np.exp(-mut1[i]*L1)[...,None],
                    L3,mut3[i][...,None],ew3[i,0],ev3[i,0],eps3[i]*np.exp(-mut1[i]*L1-mut2[i]*L2)[...,None]
                    )
                c_p3_rev[i] = self.calc_c_semi_marshak_3L_P3(
                    L3,mut3[i][...,None],ew3[i,0],ev3[i,0],eps3[i],
                    L2,mut2[i][...,None],ew2[i,0],ev2[i,0],eps2[i]*np.exp(-mut3[i]*L3)[...,None],
                    L1,mut1[i][...,None],ew1[i,0],ev1[i,0],eps1[i]*np.exp(-mut3[i]*L3-mut2[i]*L2)[...,None]
                    )
        ...
        ci_1_rev = c_p3_rev[...,16:20]
        cr_1_rev = c_p3_rev[...,20:24]
        ...
        ci_2_rev = c_p3_rev[...,8:12]
        cr_2_rev = c_p3_rev[...,12:16]
        ...
        ci_3_rev = c_p3_rev[...,0:4]
        cr_3_rev = c_p3_rev[...,4:8]
        ...
        if(z < self.l1):
            ...
            t1_rev = np.exp(-zr_rev/ew1)
            t2_rev = np.exp((zr_rev-L1)/ew1)
            t3_rev = np.exp(-mut1*zr_rev)*d1_rev

            res2 = np.sqrt(4.0*np.pi)*(np.sum(V00_1*(ci_1_rev*t1_rev+cr_1_rev*t2_rev),axis=-1)+eps1[...,0]*t3_rev)
            res2 += t3_rev/self.mu0

            res = self.Q1o*res1+self.Q1u*res2
            ...
            t1_rev = np.exp(-zr_rev/ew2)
            t2_rev = np.exp((zr_rev-L2)/ew2)
            t3_rev = np.exp(-mut2*zr_rev)*d2_rev

            res2 = np.sqrt(4.0*np.pi)*(np.sum(V00_2*(ci_2_rev*t1_rev+cr_2_rev*t2_rev),axis=-1)+eps2[...,0]*t3_rev)
            res2 += t3_rev/self.mu0

            res = self.Q2o*res1+self.Q2u*res2
            ...
            t1_rev = np.exp(-zr_rev/ew3)
            t2_rev = np.exp((zr_rev-L3)/ew3)
            t3_rev = np.exp(-mut3*zr_rev)

            res2 = np.sqrt(4.0*np.pi)*(np.sum(V00_3*(ci_3_rev*t1_rev+cr_3_rev*t2_rev),axis=-1)+eps3[...,0]*t3_rev)
            res2 += t3_rev/self.mu0

            res = self.Q3o*res1+self.Q3u*res2
